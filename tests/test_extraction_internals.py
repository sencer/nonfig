...
def test_nested_config_instantiation_failure() -> None:
  ...
  inner_type, constraints, _, _ = unwrap_hyper(Annotated[BrokenConfig, HyperMarker])
...
def test_unwrap_hyper() -> None:
  ...
  ann = Hyper[int]
  inner, constraints, is_leaf, overrides = unwrap_hyper(ann)
  assert inner is int
  assert constraints == ()
  ...
  inner2, constraints2, is_leaf2, overrides2 = unwrap_hyper(ann2)
